...
class Net(nn.Module):
	#???定义一个???
#输入的是一个图像，有高和宽，channels通常是指RGB中的三原色，此时channels通常是3。此时还有一种图像是
#monochrome图片，他的channels是1。通常意义上来讲,channels说的是颜色通道。
#其中monochrom是纯灰度图片
#in_channel是图片的颜色通道数、channels是卷积核的通道数、out_channels是输出数据的层数。
#函数定义了self、图像宽度、图像高度、图像输出宽度和图像输出高度

#__init__方法不针对对象，可以自己定义一个对象在方法内赋值。（详情见test整理）
